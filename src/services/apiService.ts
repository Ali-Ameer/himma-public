...
): Promise<ICitiesSelectData> {
  try {
    const response = await axios.get<ICitiesSelectData>(
      `${baseURL}/Governorates/${governorateId}`
    );
    return response.data;
  } catch (error) {
...
export const uploadFile = async (file: File) => {
  try {
    ...
    const response = await axios.post(
      `${baseURL}/MartyrIdForms/Attachment`,
      formData,
      {
        headers: {
    ...
    if (error.response) {
      ...
      errorMessage =
        "No response received from the server. or file too size large";
    } else {
...
export const SaveForm = async (payload: IdentityFormValues) => {
  try {
    ...
    if (error.response.data && error.response.data?.responseErrors?.length) {
      errorMessage = error.response.data?.responseErrors?.map(
        (error: any) => error?.detailMessage
      );

      console.log(
        "err list ",
        error.response.data?.responseErrors?.map(
          (error: any) => error?.detailMessage
        )
      );
    } else {
